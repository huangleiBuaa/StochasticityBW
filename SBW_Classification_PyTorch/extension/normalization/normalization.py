...
from .cdWhitening import CDWhitening
...
from .cdWhiteningSigma import CDWhiteningSigma
...
class _config:
    norm = 'BN'
    norm_cfg = {}
    norm_methods = {'No': _IdentityModule, 'BN': _BatchNorm, 'GN': _GroupNorm, 'LN': _LayerNorm, 'IN': _InstanceNorm, 'None': None, 'DBN': DBN, 'PCA': PCAWhitening, 'CD': CDWhitening, 'ItN': IterNorm, 'DBNSigma': DBNSigma, 'PCASigma': PCAWhiteningSigma, 'CDSigma': CDWhiteningSigma, 'ItNSigma': IterNormSigma}
...
def getNormConfigFlag():
    ...
    if str.find(_config.norm,'DBN') > -1 or str.find(_config.norm,'CD') > -1 or str.find(_config.norm,'PCA') > -1:
        flag += '_NC' + str(_config.norm_cfg.get('num_channels'))
