...
__all__ = ['cdWhitening', 'CDWhitening']



class CDWhitening_Single(torch.nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(CDWhitening_Single, self).__init__()
        # assert dim == 4, 'CDWhitening is not support 2D'
        self.eps = eps
...
class CDWhitening(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
        ...
        self.CDWhitening_Groups = torch.nn.ModuleList(
            [CDWhitening_Single(num_features = self.num_channels, eps=eps, momentum=momentum) for _ in range(self.num_groups-1)]
        )
        num_channels_last=self.num_features - self.num_channels * (self.num_groups -1)
        self.CDWhitening_Groups.append(CDWhitening_Single(num_features = num_channels_last, eps=eps, momentum=momentum))

        print('CDWhitening-------m_perGroup:' + str(self.num_channels) + '---nGroup:' + str(self.num_groups))
        self.affine = affine
        self.dim = dim
        shape = [1] * dim
        shape[1] = self.num_features
    ...
    def forward(self, X: torch.Tensor):
        ...
        for i in range(self.num_groups):
            X_hat_tmp = self.CDWhitening_Groups[i](X_splits[i])
...
if __name__ == '__main__':
    ItN = CDWhitening(8, num_channels=8, momentum=1, affine=False)
    print(ItN)
    ItN.train()
